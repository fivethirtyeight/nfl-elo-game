...
class Util:
    ...
    def read_games(file):
        ...
        for game in games:
            game["season"], game["neutral"], game["playoff"] = (
                int(game["season"]),
                int(game["neutral"]),
                int(game["playoff"]),
            )
            game["score1"], game["score2"] = (
                int(game["score1"]) if game["score1"] != "" else None,
                int(game["score2"]) if game["score2"] != "" else None,
            )
            game["elo_prob1"], game["result1"] = (
                float(game["elo_prob1"]) if game["elo_prob1"] != "" else None,
                float(game["result1"]) if game["result1"] != "" else None,
            )
    ...
    def evaluate_forecasts(games):
        ...
        for game in forecasted_games:
            ...
            elo_brier = (rounded_elo_prob - game["result1"]) * (
                rounded_elo_prob - game["result1"]
            )
            ...
            my_brier = (rounded_my_prob - game["result1"]) * (
                rounded_my_prob - game["result1"]
            )
            my_points = 25 - (100 * my_brier)
            my_points = round(
                my_points + 0.001 if my_points < 0 else my_points, 1
            )  # Round half up
            if game["playoff"] == 1:
                my_points *= 2
        ...
        for season in my_points_by_season:
            print(
                "In %s, your forecasts would have gotten %s points. Elo got %s points."
                % (
                    season,
                    round(my_points_by_season[season], 2),
                    round(elo_points_by_season[season], 2),
                )
            )
        ...
        if len(upcoming_games) > 0:
            ...
            for game in upcoming_games:
                print(
                    "%s\t%s vs. %s\t\t%s%% (Elo)\t\t%s%% (You)"
                    % (
                        game["date"],
                        game["team1"],
                        game["team2"],
                        int(round(100 * game["elo_prob1"])),
